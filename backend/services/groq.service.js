...
class GroqService {
   ...
   */
  getSystemPrompt(isMultilingual = false) {
...
{
  "action": "create" | "delete" | "update_priority" | "complete",
  "type": "task" | "meeting",
  "title": "string",
  "description": "string",
  ...
  "targetTitle": "keyword to find existing task/event (for delete/update actions)",
...
The user may write in Urdu or Roman Urdu. Understand and parse correctly. Always return JSON in English regardless of input language.`;
   ...
   */
  async parseTaskWithAI(message, isMultilingual = false) {
    try {
      const systemPrompt = this.getSystemPrompt(isMultilingual);
